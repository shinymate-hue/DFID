파이썬의 공식 스타일 가이드인 PEP 8에서 요청하신 주요 항목들에 대한 핵심 요약과 예제입니다.

1. 들여쓰기 (Indentation)
원칙: 공백(Space) 4칸을 사용합니다. 탭(Tab)보다는 공백 사용을 권장하며, 두 방식을 섞어서 사용하면 안 됩니다.
...
x = ( y + z ) * ( a-b ) 
3. 빈 줄 (Blank Lines)
원칙: 최상위(Top-level) 함수와 클래스 정의 사이에는 두 줄을 띄웁니다. 클래스 내부의 메서드 사이에는 한 줄을 띄웁니다.
...
원칙: 변수와 마찬가지로 소문자로 작성하며, 가독성을 위해 필요한 경우 **밑줄(_)**로 단어를 구분합니다.
